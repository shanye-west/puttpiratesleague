# Putt Pirates Golf PWA — Copilot Instructions
...
1. **Production is the only environment.** There is no staging — every deploy and every non-emulator run touches live league data (Firebase project `puttpiratesgolf`). Confirm before writing to Firestore or deploying.
