...
- **Locks are automatic.** League months aren't played on a calendar (makeups run months late), so nothing locks on a date. **`autoLockOnFinish`** locks a match on the write where it becomes finished — `completed` (closed + all 18 holes in) or closed from a `manualResult` — and when every match in the month is then locked it sets `round.locked` (which publishes the recap). All lock changes go through `setMatchLockAndSyncRound` (`helpers/matchLock.ts`): admin `setMatchLock` keeps the month in step (unlocking a match reopens its month only when every other match is locked), an admin unlock sticks (the trigger is edge-triggered), and `adminClearMatchResult` unlocks. A manual month lock via `updateRound` still works as a deadline/forfeit.
...
- **Scoring/stats triggers** (`onWrite`): `computeMatchOnWrite`, `updateMatchFacts`, `aggregatePlayerStats`, `computeRoundTotals`; **`autoLockOnFinish`** (match/month locks, see League). (`computeRoundSkins` is Cup-only and **not exported/deployed** — league rounds have no course or skins pot, so it only cost a read per score write.)
...
- **Routing**: `main.tsx`. Public: `/` (League home = standings), `/matches` (the season month by month; `?month=<roundId>`; `/season` redirects here), `/round/:id` (a month, with hole-by-hole trackers), `/round/:id/recap`, `/match/:id`, `/teams`, `/leaderboard`, `/sportsbook`, `/chat`, `/player/:id`, `/history`, `/tournament/:id`, `/settings/notifications`, `/login`; admin subtree under `/admin` gated by `RequireAdmin`.
- **League UI**: `components/league/` (`LeagueMatches` — the Matches tab: month chips → that month's team points + matches, `StandingsTables`, `MonthMatchList`, `MonthTeamPoints`), `components/MonthPicker.tsx` (month chips shared with Bets), `hooks/useLeagueSeason.ts` (rounds + matches with `splitLockedRounds` — lock finished months so only the current month keeps a live listener; for the active season it reads the shared `SeasonDataContext`), `components/match/MatchSetupPanel.tsx`, `utils/leagueTeams.ts`. `App.tsx` and `routes/Tournament.tsx` fall back to the two-sided Cup scoreboard only for a tournament without `leagueTeams`.
