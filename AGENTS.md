...
- **Cards are saved whole.** `saveCaptainsMatchRound` validates a card and replaces `rounds.{n}` wholesale (a `FieldPath` update), copying the course's name and tees when it was played on an app course. Strokes are stored per hole (`strokesA`/`strokesB`, 0/1): the admin page places them on the hardest holes from the course's handicap indexes, or they're ticked by hand. Players lock once any card exists.
